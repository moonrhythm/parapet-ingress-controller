...
// LOCAL PATCH (not upstream pingora 0.8): idle timeout for downstream H2
// connections. Upstream's H2 serve loop has no idle timeout (its `TODO: add a
// timeout?`), so a client/scanner that completes the handshake and then sends no
// streams — or whose TCP goes half-open without a RST — keeps the whole H2
// connection (read buffers, per-read `pingora_timeout` futures, handshake state)
// alive forever. Under a flood of such connections that is an unbounded heap
// leak (confirmed by jemalloc heap profiling). Default 60s, matching the H1
// keepalive default; override with PINGORA_H2_IDLE_TIMEOUT_SECS (read once).
fn h2_idle_timeout() -> Duration {
    static T: std::sync::OnceLock<Duration> = std::sync::OnceLock::new();
    *T.get_or_init(|| {
        let secs = std::env::var("PINGORA_H2_IDLE_TIMEOUT_SECS")
            .ok()
    ...
    T: HttpServerApp + Send + Sync + 'static,
{
    async fn process_new(
        ...
        if h2c || matches!(stream.selected_alpn_proto(), Some(ALPN::H2)) {
            ...
            // LOCAL PATCH: count in-flight streams so the idle timeout below can
            // tell a truly idle connection from one busy serving long-lived
            // streams (SSE / long-poll), which must NOT be closed.
            let active = Arc::new(AtomicUsize::new(0));
            loop {
                // this loop ends when the client closes the h2 conn, or (LOCAL
                // PATCH) when it sits idle with no in-flight streams for
                // H2_IDLE_TIMEOUT — see the const above.
                let h2_stream = tokio::select! {
                    biased; // prefer serving a ready stream over the idle timer
                    ...
                    _ = pingora_timeout::fast_timeout::fast_sleep(h2_idle_timeout()) => {
                        // No new stream within the idle window. If nothing is
                        // in flight either, the connection is idle/half-open;
                        // returning None drops `h2_conn`, closing the socket
                        // immediately (no graceful GOAWAY wait, which could hang
                        // on a dead peer). Otherwise keep serving.
                ...
                // LOCAL PATCH: mark a stream in flight; the guard clears it when
