// Package cache is the edge's optional disk-backed HTTP response cache (Phase 4,
// EDGE_CACHE_*, off by default). It mirrors the Rust edge's honor-origin policy
// (rust/edge/src/diskcache.rs + proxy.rs): caches only when the origin opts in
// via explicit Cache-Control/Expires freshness; refuses private/no-store/
// no-cache, Set-Cookie, and Vary: *; honors Vary; GET/HEAD only; ignores the
// client's request Cache-Control (CDN-style, so a client can't bust the shared
// cache); LRU-bounded by total bytes with a per-object cap; restart-persistent;
// fail-static. Sets X-Cache: HIT|MISS.
//
// One deliberate divergence from the Rust/pingora edge: a GET response is cached
// only when it carries a Content-Length within the per-object cap. The cache
// commits a body only once written bytes == Content-Length, which guarantees a
// truncated response (client disconnect, upstream error) is never stored — Go's
// httputil.ReverseProxy gives no clean end-of-stream signal to a wrapping
// ResponseWriter, so a chunked (no Content-Length) GET passes through uncached
// rather than risk caching a partial body. HEAD has no body and is unaffected.
package cache
...
// cacheableMethods are the only methods the cache engages for.
...
// cacheableStatus is the set of status codes the edge will cache (a conservative
...
func decide(method string, status int, h http.Header, maxFileSize int64, now time.Time) decision {
	...
	// Clamp absurd freshness so now+ttl stays well within time.UnixNano's range
	// (~year 2262); past that the stored FreshUntil wraps and the entry would be
	// reaped on its first lookup (cached but never served). 10y is far beyond any
	// real max-age.
	if ttl > maxTTL {
		ttl = maxTTL
	}
	// For GET, require a Content-Length within the cap (see package doc).
