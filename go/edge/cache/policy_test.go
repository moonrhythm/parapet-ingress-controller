...
func cacheableGET(t *testing.T, h http.Header) bool {
	...
	if h.Get("Content-Length") == "" {
		h.Set("Content-Length", "10") // GET needs a CL to be cacheable
	}
...
func TestDecide_HonorsOriginFreshnessOnly(t *testing.T) {
	...
	// no freshness directive -> NOT cached (no heuristic/forced TTL)
...
func TestDecide_Expires(t *testing.T) {
	...
	// unparseable Expires (e.g. "0") -> already expired
