...
| `HOST_RPS` | `0` | Per-Host admits per **1s epoch-aligned fixed window** per replica (0 = off). Up to 2× `HOST_RPS` across a window boundary. Hosts `IsKnownHost` does not serve — including traffic hitting **host-less catch-all** rules — share one `other` bucket. Overflow is 503 (ceiled `Retry-After`) before WAF/origin. Independent of `HOST_CONCURRENT_*`. N replicas admit up to N×. Host RPS and host concurrent 503s short-circuit before the access log and `parapet_requests` — observe them on `parapet_ratelimit_total`, `parapet_rejected_requests`, and `parapet_host_ratelimit_requests`. The edge has the same fuse as `EDGE_HOST_RPS` (see [EDGE.md](EDGE.md)). |
