...
| `HOST_RPS` | `0` | Per-Host admits per **1s epoch-aligned fixed window** per replica (0 = off). Up to 2× `HOST_RPS` across a window boundary. Hosts `IsKnownHost` does not serve — including traffic hitting **host-less catch-all** rules — share one `other` bucket. Overflow is 503 (ceiled `Retry-After`) before WAF/origin. Independent of `HOST_CONCURRENT_*`. N replicas admit up to N×. On the **controller**, host RPS and host concurrent 503s short-circuit before the access log and `parapet_requests` — observe them on `parapet_ratelimit_total`, `parapet_rejected_requests`, and `parapet_host_ratelimit_requests`. The edge has the same fuse as `EDGE_HOST_RPS` (see [EDGE.md](EDGE.md)); edge 503s **are** counted on `parapet_requests` because that middleware sits outside the limiter. |
| `HOST_CONCURRENT_CAPACITY` / `_SIZE` | `0` | Per-host in-flight cap / queue size. Slot is released when upstream response headers arrive (or on a 101 upgrade), not at end-of-body — so SSE / WebSocket / long-poll streams don't pin a slot for the stream lifetime. The cap exists to shed load while upstreams are *unresponsive*. The bucket key is `hostlabel.Of` (unknown Hosts share `other`), same collapse as `HOST_RPS`. |
| `HOST_COUNTRY_CONCURRENT_CAPACITY` / `_SIZE` | `0` | Per-host+country cap / queue (same release semantics and Host collapse as `HOST_CONCURRENT_CAPACITY`) |
