...
| `parapet_coraza_matches{rule_id,severity,scope,zone}` | Coraza (OWASP CRS / SecLang) rule matches that engage logging (`nolog` administrative CRS matches excluded; detections and 949110 count in enforce and detect mode); `scope` = `global\|zone`, `zone` = zone registry key `<ns>/<name>` (`""` for global; rule ids are shared CRS ids, so `zone` attributes a match), no `_total` suffix |
