...
On a **dial failure** (no connection established), the proxy's `ErrorHandler` panics; `retryMiddleware` (`retry.go`) recovers it and retries the request (up to 5 times with backoff) — but only when the body hasn't been read, so non-idempotent requests aren't replayed. `http.ErrAbortHandler` (ReverseProxy body-copy abort after headers were written) is re-panicked so net/http can close the connection without treating it as a handler crash. A **post-connect** failure that produces no HTTP response (reset, EOF, header timeout, TLS handshake error) 502s that request and **marks the pod bad** for 2s so the next Lookup skips it — it is **not** retried, because the request may already have reached the pod. `proxy.IsRetryable` is **dial-error-only**: an upstream that *responds* — including 502/503 — has processed the request, so its response passes through to the client unchanged rather than being retried or marking the pod (retrying could duplicate side effects and amplify load on a failing backend). See [`SPEC.md`](SPEC.md).
