...
| `parapet_edge_ca_rotation_stuck` | CP | gauge 0/1 | — | 1 once the CA Secret has sat in the `OLD++NEW` overlap past `EDGE_CA_ROTATION_DEADLINE` — a half-applied rotation (the rotated-out edge is still trusted). The overlap-start is read from a CA-Secret annotation (`edge-ca-rotation-started`, stamped by RotateCA), so the age is **restart-immune and replica-identical** — a CP restart mid-overlap doesn't reset the clock. **Page on this.** |
| `parapet_edge_token_disabled_without_rotation` | CP | gauge=1 | `edge_id` | 1 per blacklisted token id — the bare-blacklist-isn't-revocation reminder (a disabled token's already-issued leaf is trusted until the CA is rotated out). Never a false all-clear; remove the tombstone after rotating to clear it. |
...
| `EDGE_TRUST_READY_WAIT` | core | `10s` | Cold-start readiness gate. On startup the core blocks reporting **Ready** until the edge-CA pool loads, up to this bound, so the edge isn't routed traffic during the window where `:443` sends no `CertificateRequest` and a mTLS-only edge (no CIDR cover) would stay untrusted on any connection it opens until that connection recycles. The wait runs **after** the k8s preload, by which point the parallel trust fetch has usually landed, so it is normally a no-op. **Fail-static:** on timeout the core reports Ready anyway and serves **CIDR-only** until the bundle loads (the edge converges to mTLS-trusted then) — a CP outage at boot never blocks serving. Set `0` to disable the wait (the pre-gate behavior). Only meaningful when the edge has no CIDR cover; with a `TRUST_PROXY` CIDR for the edge the window has no effect. **Precondition:** the gate only closes the window if edge→controller traffic is routed by a layer that honors pod readiness — the shipped `deploy/02-service.yaml` (`type: LoadBalancer` + `externalTrafficPolicy: Local`) qualifies, but if the edge dials a pod IP / headless Service / `hostNetwork` directly, the listener accepts during the wait and the gate is a no-op. **Availability trade:** the value is a correctness-vs-availability knob — during a CP outage every (re)starting pod stays NotReady for the full bound before serving CIDR-only (delaying *all* ingress, not just edge), so keep it short. |
...
| `EDGE_CA_ROTATION_DEADLINE` | CP (serving) | `86400` (s, =24h) | How long the CA Secret may sit in the `OLD++NEW` overlap before `parapet_edge_ca_rotation_stuck` pages. Size it above a full revoke's expected duration (both convergence gates); `0` disables the gauge. |
