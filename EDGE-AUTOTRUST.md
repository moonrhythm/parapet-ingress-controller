...
| `parapet_edge_ca_rotation_stuck` | CP | gauge 0/1 | — | 1 once the CA Secret has sat in the `OLD++NEW` overlap past `EDGE_CA_ROTATION_DEADLINE` — a half-applied rotation (the rotated-out edge is still trusted). `max()` across replicas is robust to a single replica restart. **Page on this.** |
